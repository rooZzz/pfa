...
**Sibling table `pfa.tax_constants`** (UK tax/legal constants: allowances, rates, bands, access ages) exists on disk but is deliberately not a text-to-SQL target. It is injected into the advice and briefing payload via the deterministic accessor in `server/tax/constants.ts` (`resolveConstant`, `taxConstantsForDate`), never queried by generated SQL — its dated, status-tagged temporal logic is too easy to get wrong in generated queries.
