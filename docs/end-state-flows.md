...
**What the model does.** The app — not the harness — evaluates every directive across all active goals against current data and pushes the complete observation set to the harness. The harness prioritises and frames; it never decides what to query. Coverage is the app's responsibility, framing is the harness's. A directive whose metric cannot resolve (no data captured yet) fires as a data-gap directive, which becomes the next capture prompt. Because goals share one balance sheet, the briefing also evaluates cross-goal contention: where two goals claim the same accounts it emits a `contention` directive over the shared pool ("the same money backs the safety net and the deposit; it cannot fund both in full"). Detecting the overlap is deterministic and an observation; resolving it is advice.

**Hypotheticals.** A "what if" question (a bonus, an extra contribution, a windfall) is served by the conditional engine, not by the harness reasoning over balances. The harness composes the hypothesis as an overlay of rows the schema already holds — a real event expressed as the rows it produces — and the app recomputes the full briefing under that overlay (`evaluate_scenario`), so every projected figure is grounded, not hand-computed. The harness may show each scenario's grounded outcome (the frontier); ranking them or naming the one to pick stays advice (Flow 7). Structural hypotheticals that cannot be expressed as overlay rows (a rate change, a scheme change) fall back to harness reasoning, flagged as assumption-based.

**Tax position.** The briefing also pushes a grounded `tax_position` block: the app joins the stored UK rulebook (`tax_constants`) to the user's actual income (salary, payslips, RSU vesting) and computes projected annual income, marginal rate, the ~60% personal-allowance taper band, an income-tax and NI estimate, and the pension annual-allowance taper — deterministic arithmetic, never the harness's to hand-compute or the user's to self-report. A bonus or windfall flows through the same engine via an `income_events` overlay on `evaluate_scenario`, so "what's the net of my bonus / what does it do to my pension allowance" is answered from data, not asked. It is observation only — facts and flags, never a recommendation. Realized (payslip year-to-date) and projected (full-year) stay separate, labelled figures. CGT, dividends, and the standing total-comp picture are deferred.

**Read / written.** Reads goals, metric definitions, and all data stores. No writes. A scenario evaluation clones the touched read tables into a throwaway schema and tears it down after; the canonical store is never mutated. Whether briefings are persisted is an open decision.

**What the user sees.** A prioritised set of grounded observations ("ISA 60 percent funded, 47 days left"; "house deposit goal set, no savings account linked"; "ISA allowance drops on [date], 73 days away — proposed, subject to legislation"; "the same £35k backs both the emergency fund and the deposit"), each traceable to its data or named as a gap. Deadline directives fire from pending tax constants the same way as from the tax-year boundary, and a directive on an announced-not-enacted change carries its certainty. No ranking of options — that is advice, Flow 7.
