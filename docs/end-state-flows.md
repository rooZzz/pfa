...
**What the user sees.** A prioritised set of grounded observations ("ISA 60 percent funded, 47 days left"; "house deposit goal set, no savings account linked"), each traceable to its data or named as a gap. No ranking of options — that is advice, Flow 7.
...
- Dated domain assumptions baked into the catalog — safe-withdrawal-rate default, pension access age, annual ISA and pension allowances — and how they version over time.
